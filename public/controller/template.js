// Endpoint GET
export let UrlGetPendaftar = "https://komarbe.ulbi.ac.id/pendaftar/pendaftar";
export let UrlGetAdminLevel = "https://komarbe.ulbi.ac.id/admin_level";
export let UrlGetAdmin = "https://komarbe.ulbi.ac.id/admin";
export let UrlGetJalur = "https://komarbe.ulbi.ac.id/jalur";
export let UrlGetFakultas = "https://komarbe.ulbi.ac.id/fakultas";
export let UrlGetProdi = "https://komarbe.ulbi.ac.id/prodi";
export let UrlGetAgama = "https://komarbe.ulbi.ac.id/agama";
export let UrlGetTahunAkademik = "https://komarbe.ulbi.ac.id/tahun_akademik";

// Endpoint GET by Id
export let UrlGetPendaftarById = "https://komarbe.ulbi.ac.id/pendaftar/pendaftar/get";
export let UrlGetAdminLevelById = "https://komarbe.ulbi.ac.id/admin_level/get";
export let UrlGetAdminById = "https://komarbe.ulbi.ac.id/admin/get";
export let UrlGetJalurById = "https://komarbe.ulbi.ac.id/jalur/get";
export let UrlGetFakultasById = "https://komarbe.ulbi.ac.id/fakultas/get";
export let UrlGetProdiById = "https://komarbe.ulbi.ac.id/prodi/get";
export let UrlGetAgamaById = "https://komarbe.ulbi.ac.id/agama/get";
export let UrlGetTahunAkademikById = "https://komarbe.ulbi.ac.id/tahun_akademik/get";

// Endpoint POST
export let UrlPostAdminLevel = "https://komarbe.ulbi.ac.id/admin_level/post";
export let UrlPostAdmin = "https://komarbe.ulbi.ac.id/admin/post";
export let UrlPostJalur = "https://komarbe.ulbi.ac.id/jalur/post";
export let UrlPostFakultas = "https://komarbe.ulbi.ac.id/fakultas/post";
export let UrlPostProdi = "https://komarbe.ulbi.ac.id/prodi/post";
export let UrlPostTahunAkademik = "https://komarbe.ulbi.ac.id/tahun_akademik/post";

// Endpoint PUT
export let UrlPutAdminLevel = "https://komarbe.ulbi.ac.id/admin_level/put";
export let UrlPutAdmin = "https://komarbe.ulbi.ac.id/admin/put";
export let UrlPutJalur = "https://komarbe.ulbi.ac.id/jalur/put";
export let UrlPutFakultas = "https://komarbe.ulbi.ac.id/fakultas/put";
export let UrlPutProdi = "https://komarbe.ulbi.ac.id/prodi/put";
export let UrlPutTahunAkademik = "https://komarbe.ulbi.ac.id/tahun_akademik/put";

// Endpoint DELETE
export let UrlDeleteAdminLevel = "https://komarbe.ulbi.ac.id/admin_level/delete";
export let UrlDeleteAdmin = "https://komarbe.ulbi.ac.id/admin/delete";
export let UrlDeleteJalur = "https://komarbe.ulbi.ac.id/jalur/delete";
export let UrlDeleteFakultas = "https://komarbe.ulbi.ac.id/fakultas/delete";
export let UrlDeleteProdi = "https://komarbe.ulbi.ac.id/prodi/delete";
export let UrlDeleteAgama = "https://komarbe.ulbi.ac.id/agama/delete";
export let UrlDeleteTahunAkademik = "https://komarbe.ulbi.ac.id/tahun_akademik/delete";
